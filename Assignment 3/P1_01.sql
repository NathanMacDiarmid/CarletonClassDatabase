...
CREATE TABLE subs (
    subName text primary key NOT NULL, --name of the subsriber
    subAddress text NOT NULL, --address of subscriber
    subNumber text NOT NULL --phone number of subscriber
);

INSERT INTO subs 
SELECT subscribers.name, subscribers.address, lines.areacode || '-' || lines.officecode || '-' || lines.stationcode 
FROM subscribers 
INNER JOIN lines ON subscribers.portid = lines.portid;

SELECT * FROM subs;

/*
Test Output:
subName      subAddress    subNumber   
-----------  ------------  ------------
